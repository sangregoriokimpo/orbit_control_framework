...
class TwoBodyRK4:
    ...
    def rk4_step(self, r: Vec3, v: Vec3, dt: float, a_cmd: Vec3 = (0.0, 0.0, 0.0)):
        """One RK4 step for dr/dt=v, dv/dt=gravity(r)+a_cmd."""
        ...
        r_next = add(
            r,
            mul(dt / 6.0, add(add(k1r, mul(2.0, k2r)), add(mul(2.0, k3r), k4r))),
        )
        v_next = add(
            v,
            mul(dt / 6.0, add(add(k1v, mul(2.0, k2v)), add(mul(2.0, k3v), k4v))),
        )
...
def v_dot(a: Vec3, b: Vec3) -> float:
    return a[0]*b[0] + a[1]*a[1] + a[2]*a[2]
